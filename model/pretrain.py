...
class ESM2(nn.Module):
    ...
    #     assert tokens is not None or one_hot is not None
    
    #     # ===== 1. padding mask =====
    #     if tokens is not None:
    #         padding_mask = tokens.eq(self.padding_idx)
    #     else:
    #         # 假设 one_hot padding 是全0
    #         padding_mask = one_hot.sum(dim=-1) == 0  # [B, L]
    
    #     # ===== 2. embedding =====
    #     if one_hot is not None:
    #         # 🔥 核心：one-hot → embedding
    #         embed_weight = self.embed_tokens.weight  # [V, D]
    #         x = torch.matmul(one_hot, embed_weight)  # [B, L, D]
    #     else:
    ...
    #     # ===== 3. token dropout（可选）=====
    #     if tokens is not None and self.token_dropout:
    #         x.masked_fill_((tokens == self.mask_idx).unsqueeze(-1), 0.0)
...
def _has_regression_weights(model_name):#"esm2_t33_650M_UR50D"
